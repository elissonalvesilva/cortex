...
func (c *Command) MustInit() {
	...
	for _, command := range commands {
		...
		if len(command.Short) == 0 {
			command.Short = flagsx.Get(commandKey + ".short")
		}
		if len(command.Long) == 0 {
			command.Long = flagsx.Get(commandKey + ".long")
		}
		if len(command.Example) == 0 {
			command.Example = flagsx.Get(commandKey + ".example")
		}
		command.Flags().VisitAll(func(flag *pflag.Flag) {
			flag.Usage = flagsx.Get(fmt.Sprintf("%s.%s", commandKey, flag.Name))
		})
